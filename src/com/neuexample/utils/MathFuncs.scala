...
//    println(x3+"--"+x3IntPart+"--"+x3DecPart+"--"+array(x3IntPart));
  ...
  def calculateSoc(maxTemperature:Integer,maxCellVoltage:Integer): Double ={

    val voltageArray = socvoltageArray(getTemperatureIndex(maxTemperature))
//     println(voltageArray.mkString(","))

    if(maxCellVoltage <= voltageArray(voltageArray.length-1) ){
      return socNums(voltageArray.length-1);
    }
    if( maxCellVoltage >= voltageArray(0) ){
      return socNums(0);
    }

    for( i <- 1 until voltageArray.length   ){
      if( maxCellVoltage >= voltageArray(i)  ){
        return linearInsertion( voltageArray(i-1),socNums(i-1),voltageArray(i),socNums(i),maxCellVoltage  )
      }
    }
    0
  }
  ...
  def calculateSoc(maxTemperature:Double,maxCellVoltage:Integer): Double ={
